...
// xor_key: известный открытый префикс (handshake) XOR-им с началом шифртекста,
// получаем K байт ключа, затем расшифровываем сообщения повторяющимся ключом.
namespace algo {
...
inline std::string xor_strings(std::string str1, std::string str2, std::size_t n) {
    std::string result;
...
inline std::string find_key(std::size_t key_length, std::string handshake, std::string ciphertext) {
    ...
    return xor_strings(handshake, ciphered_handshake_in_bytes, key_length);
}
...
inline std::string decipher_text(std::string key, std::string ciphertext) {
    ...
    for (std::size_t i = 0; i < ciphertext_in_bytes.length(); ++i) {
        std::size_t j = i % key.length();
